...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse && LaunchState.shared.browserDestination == nil {
    ...
    private func finishLaunchLoader() {
        ...
        if LaunchState.shared.noInternetMessage != nil {
            presentNoInternetThenSwitchToNative(window: window, navigationController: navigationController)
            return
        }
    ...
    private func presentNoInternetThenSwitchToNative(window: UIWindow, navigationController: UINavigationController) {
        guard let loaderVC = window.rootViewController else { return }
        let message = LaunchState.shared.noInternetMessage ?? "No internet connection. Please turn on the internet and open the app again."
        ...
        loaderVC.present(alert, animated: true)
    }
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        // Resend will be triggered by messaging(_:didReceiveRegistrationToken:) after Firebase
        // re-issues the FCM token in response to the new APNs token.
    }
